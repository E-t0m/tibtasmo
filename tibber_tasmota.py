...
def main():
	...
	price_ut = price_avg #- (price_spread /5 )		# it sets the threshold for the timers
			...
			if price_lt > cur_price: 				# lower threshold
				...
				if p_hour_is_cur:
					if hot_off: pass 
						...
						msg += 'hot off:'
						hot_off = True
						msg += ' 1' if tasmota_switch( tasmota_dev['auto1_off'],'0').status_code == 200 else ' 1 error'
						msg += ' 2' if tasmota_switch( tasmota_dev['auto2_off'],'0').status_code == 200 else ' 2 error'
						if not verbose: syslog.syslog(syslog.LOG_INFO, msg)
				else:
	...
	if True: 
		with open('/home/vzlogger/timer.txt','w') as fo:
			...
			for tib_form in future_prices:
				file_form = tib_form.replace('T',' ') + ':00:00 ' + ('1200 1200' if tib_form in zerotimer else '0 0')
				fo.write(file_form+'\n')
